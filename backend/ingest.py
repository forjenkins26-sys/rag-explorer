...
from vector_store import add_chunks, delete_source, list_sources
...
def ingest_pdf(doc_path: Path) -> dict:
    """Read, chunk, embed, and store one document. Idempotent per filename.
    Despite the name, handles any type in SUPPORTED_EXTENSIONS (pdf/txt/md/xlsx/xls)."""
    source_name = doc_path.name
    delete_source(source_name)  # re-ingest cleanly if file changed
    ...
    for idx, chunk in enumerate(chunks):
        ...
        metadatas.append(
            {"source": source_name, "page": chunk["page"], "chunk_index": idx}
        )
